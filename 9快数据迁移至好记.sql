...
BEGIN
    ...
    SET @query = CONCAT(
        ...
        'old.name, ',
        'old.default_currency_code, ',
        'IF(old.default_expense_account_id IS NULL, NULL, old.default_expense_account_id + @account_shift), ',  
    ...
    SET @query = CONCAT(
        ...
        'IFNULL(old.account_id + @account_shift, NULL), ',  
        'CAST(old.amount AS DECIMAL(20,4)), ',
        'CAST(old.converted_amount AS DECIMAL(20,4)), ',
        'IFNULL(old.payee_id + @payee_shift, NULL), ',  
        'CASE old.type ',  
            'WHEN 100 THEN 1 ',  
            'WHEN 200 THEN 2 ',  
            'WHEN 300 THEN 3 ',  
            'ELSE 4 END, ',    
        'IFNULL(old.to_id + @account_shift, NULL), ',  
    ...
    SET @query = CONCAT(
        ...
        'SELECT ',
        'old.id + @file_shift, ',          
        'IFNULL(old.flow_id + @bill_shift, 0), ',  
